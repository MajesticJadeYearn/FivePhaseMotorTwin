...
namespace FivePhaseMotorTwin
{
    public partial class MainForm
    {
        ...
        private FlowLayoutPanel BuildLeftPanel()
        {
            ...
            GroupBox control = CreateGroup("运行模式控制", 270, 232);
            _modeCombo = new ComboBox();
            _modeCombo.DropDownStyle = ComboBoxStyle.DropDownList;
            _modeCombo.Font = AppTheme.Font(9.0f, FontStyle.Regular);
            _modeCombo.Width = 240;
            for (int i = 0; i < ScenarioInfo.All.Length; i++) _modeCombo.Items.Add(ScenarioInfo.All[i].DisplayName);
            _modeCombo.SelectedIndex = 0;
            _modeCombo.SelectedIndexChanged += OnModeChanged;
            control.Controls.Add(Place(_modeCombo, 14, 26, 240, 28));

            TableLayoutPanel buttons = new TableLayoutPanel();
            buttons.ColumnCount = 2;
            buttons.RowCount = 3;
            buttons.Location = new Point(14, 62);
            buttons.Size = new Size(240, 120);
            buttons.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50));
            buttons.ColumnStyles.Add(new ColumnStyle(SizeType.Percent, 50));
            for (int r = 0; r < 3; r++) buttons.RowStyles.Add(new RowStyle(SizeType.Absolute, 38));
            ...
            _faultButton = CreateButton("故障注入", OnFaultClicked);
            _autoButton = CreateButton("自动运行", OnAutoClicked);
            _screenshotButton = CreateButton("导出运行截图", OnScreenshotClicked);
            ...
            buttons.Controls.Add(_autoButton, 0, 2);
            buttons.Controls.Add(_screenshotButton, 1, 2);
            control.Controls.Add(buttons);
            Label hint = CreateSmallLabel("模式切换后，系统先进入健康状态，再按场景注入故障。", 14, 190, 240, 30);
